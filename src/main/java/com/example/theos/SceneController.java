...
public class SceneController {
     ...
     */
    public static void showGameBoardSceen() {
        ...
        Player testPlayer = new Player("Mint’O Lint", mintOLintDie, Paths.get("images"));
        ...
        testPlayer.setImageView(sprite4);
        testPlayer.setCurrentField(spawn4);
        testPlayer.getImageView().setX(spawn4.getX() - 27);
        testPlayer.getImageView().setY(spawn4.getY() - 27 - 15);
        ...
        scene.setOnKeyPressed(event -> {
            if (TheOs.waitingForUserInput) {
                ...
                if (event.getCode() == KeyCode.DOWN && testPlayer.getSpecialDie().getCharge() > 0) {
                    gameBoard.getDiceUI().selectSpecialDie();
                }
                if (event.getCode() == KeyCode.SPACE) {
                    TheOs.waitingForUserInput = false;
                    gameBoard.playerTurn(testPlayer);
                }
            }
        });
    }
